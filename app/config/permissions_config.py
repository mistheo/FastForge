...
class PermissionsRegister:
    ...
    # _registry maps: { model_name: { role: {"attributes": set(), "overwrite": bool } } }
    _registry: Dict[str, Dict[UserRole, Dict[str, Any]]] = {}

    @classmethod
    def register(cls, model: Type, role: UserRole, attributes: List[str], overwrite: bool = False) -> None:
        ...
        if role not in cls._registry[model_name] or overwrite:
            ...
            cls._registry[model_name][role] = {"attributes": set(attributes), "overwrite": overwrite}
        else:
    ...
    def get_permissions(cls, model: Type, role: UserRole) -> set:
        ...
        # If the model has an overwrite=True for this role, only return its attributes
...
def Permissions(role: UserRole, attributes: List[str], overwrite_attrs: bool = False):
    """Decorator to register model attribute permissions for a given user role."""
    def decorator(cls):
        PermissionsRegister.register(cls, role, attributes, overwrite=overwrite_attrs)
        return cls
    return decorator


def AllUserRolePermissions(attributes: List[str], exclude_roles: List[UserRole] = [], overwrite_attrs: bool = False):
